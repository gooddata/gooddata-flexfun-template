...
def testing_server_config_files() -> tuple[str, ...]:
    # intentionally, this uses the main flexfun config so that all
    ...
    return (os.path.join(__config_dir__, "flexfun.config.toml"),)


@pytest.fixture(scope="module")
def testing_flexfun_server(
    testing_server_config_files, testing_server_env_config
) -> Generator[gf.GoodDataFlightServer]:
    """
    This fixture starts up Flight server that hosts all FlexFunctions
    that you have implemented and added to `flexfun.config.toml`.
